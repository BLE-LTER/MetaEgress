...
create_EML <- function(meta_list, dataset_id, boilerplate, license, data_table, 
    other_entity = NULL) {
    ...
    creator_list <- subset(meta_list[["creator"]], datasetid == dataset_id & authorshiprole == 
        "creator")
    
    # function to create a creator object
    create_creator <- function(creator) {
        given_name <- trimws(paste(creator[["givenname"]], replace(creator[["givenname2"]], 
            is.na(creator[["givenname2"]]), ""), " "))
        
        p <- list(individualName = list(givenName = given_name, surName = creator[["surname"]]), 
            organizationName = creator[["organization"]], address = list(deliveryPoint = trimws(paste(creator[["address1"]], 
                replace(creator[["address2"]], is.na(creator[["address2"]]), ""), 
                replace(creator[["address3"]], is.na(creator[["address3"]]), ""), 
                " ")), city = creator[["city"]], administrativeArea = creator[["state"]], 
                postalCode = creator[["zipcode"]], country = creator[["country"]]), 
            phone = creator[["phone1"]], electronicaMailAddress = if (is.na(creator[["email"]])) NULL else creator[["email"]], 
            userId = if (!is.na(creator[["orcid"]])) list(system = paste0("https://orcid.org/", 
                creator[["orcid"]])) else NULL)
        return(p)
    }
    ...
    for (ii in 1:length(methodnum)) {
        ...
        software <- if (is.na(method_s$softwareDescription[1])) {
            ...
            list(title = if (is.na(method_s$softwareTitle)[1]) {
                NULL
            } else {
            ...
            }, implementation = list(distribution = list(online = list(url = list(method_s$softwareDescription[1], 
                `function` = list("download"))))), version = method_s$softwareVersion[1])
        }
        
        instrument <- if (is.na(method_s$instrumentDescription[1])) {
            NULL
        } else {
            list(title = if (is.na(method_s$instrumentTitle[1])) {
                NULL
            } else {
            ...
            }, distribution = list(online = list(url = list(method_s$instrumentDescription[1], 
        ...
        if (!is.na(method_s$protocolDescription)) {
            for (kk in 1:nrow(method_s)) {
                protocol <- if (is.na(method_s$protocolDescription[kk])) {
                  ...
                  list(title = if (is.na(method_s$protocolTitle[kk])) {
                    NULL
                  } else {
                  ...
                  }, distribution = list(online = list(url = list(method_s$protocolDescription[kk], 
                    `function` = list("download")))))
                }
                if (kk == 1) {
        ...
        methodstep <- list(description = set_TextType(methodnum[ii]), instrumentation = instrument, 
            software = software, protocol = protocolall)
    ...
    tempcover <- list(rangeOfDates = list(beginDate = list(calendarDate = as.character(tempo[, 
        "begindate"])), endDate = list(calendarDate = as.character(tempo[, "enddate"]))))
    # -----------------------------------------------------------------------------
    # spatial coverage
    ...
    # -----------------------------------------------------------------------------
    # overall coverage
    coverage <- list(geographicCoverage = geoall, temporalCoverage = tempcover)
    ...
    key_func <- function(keyword_list) {
        k <- list(keyword = keyword_list[["keyword"]], keywordThesaurus = if (keyword_list[["keyword_thesaurus"]] == 
            "none") NULL else keyword_list[["keyword_thesaurus"]])
        return(k)
    }
    ...
    dataset <- list(title = project[["title"]], alternateIdentifier = project[["alternateid"]], 
        shortname = project[["shortname"]], creator = personnel, pubDate = as.character(as.Date(project[["pubdate"]])), 
        intellectualRights = license, abstract = abstract, keywordSet = kall, 
        coverage = coverage, contact = contact, publisher = publisher, distribution = distribution, 
        project = project_xml, methods = method_xml, language = "English", dataTable = data_table, 
    ...
    eml <- list(packageId = project[["edinum"]], system = "knb", schemaLocation = "eml://ecoinformatics.org/eml-2.1.1 http://nis.lternet.edu/schemas/EML/eml-2.1.1/eml.xsd", 
        access = access, dataset = dataset, additionalMetadata = list(metadata = list(unitList = unit_list)))
